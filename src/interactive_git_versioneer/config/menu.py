...
def _format_size(size_bytes: int) -> str:
    """Format size in bytes to human-readable string (e.g. 2.0 GB)."""
...
def _format_ollama_date(iso_date: str) -> str:
    """Format ISO date from Ollama to relative time (e.g. '3 weeks ago')."""
    if not iso_date:
        return "-"
...
def _select_model_interactive() -> Optional[str]:
    ...
    print(
        f"{Colors.CYAN}Fetching available models...{Colors.RESET}", end="", flush=True
    )
    ...
    while True:
        ...
        print_header(
            f"SELECT MODEL  (page {page + 1}/{total_pages}  —  {total} models)"
        )
        print()
...
def run_config_menu() -> None:
    ...
    def show_config_status() -> None:
        ...
        For Ollama, also shows context length if available.
        """
        ...
        if model:
            ...
            if provider == "Ollama" and model:
                from ..core.ai import get_ollama_model_details

                details: Optional[dict] = get_ollama_model_details(model)
                if details and details.get("context_length"):
                    ctx: int = details["context_length"]
                    ctx_str: str = f"{ctx:,}" if ctx else "-"
                    print(f"{Colors.CYAN}  Context: {ctx_str} tokens{Colors.RESET}")
                else:
                    print(f"{Colors.YELLOW}  Context: unavailable{Colors.RESET}")
    ...
    def action_show_config() -> Literal[False]:
        ...
        print(
            ...
            f" · {Colors.YELLOW}Ollama{Colors.RESET}"
    ...
    def action_configure_ai() -> Literal[False]:
        ...
        """
        clear_screen()
        print_header("CONFIGURE AI (Groq/OpenRouter/OpenAI/Ollama)")
        print()
        ...
        if (
            current_model
            and current_url
            and ("localhost" in current_url or "127.0.0.1" in current_url)
        ):
            from ..core.ai import get_ollama_model_details

            details: Optional[dict] = get_ollama_model_details(current_model)
            if details and details.get("context_length"):
                ctx: int = details["context_length"]
                print(f"  Context: {Colors.CYAN}{ctx:,} tokens{Colors.RESET}")
            else:
                print(f"  Context: {Colors.YELLOW}unavailable{Colors.RESET}")
        ...
        print(f"  6. Quick setup (Ollama - local)")
        print(f"  0. Back")
        print()
        ...
        if choice == "1":
            ...
            if api_key_input:
                ...
                set_config_value("OPENAI.model", "meta-llama/llama-3.3-70b-instruct")
                print()
                print(
    ...
    def _configure_ollama() -> None:
        """Interactive Ollama configuration with model discovery."""
        clear_screen()
        print_header("CONFIGURE OLLAMA (LOCAL)")
        print()

        print(
            f"{Colors.WHITE}Ollama must be running locally. Start it with: ollama serve{Colors.RESET}"
        )
        print(
            f"{Colors.WHITE}Install models with: ollama pull <model>  (e.g. ollama pull llama3.2){Colors.RESET}"
        )
        print()

        current_url: Optional[str] = get_config_value("OPENAI.baseURL")
        default_url: str = "http://localhost:11434/v1"
        base_url: str = current_url or default_url

        print(f"{Colors.CYAN}Current Base URL: {Colors.RESET}{base_url}")
        if base_url and not base_url.endswith("/v1"):
            print(
                f"{Colors.YELLOW}Note: Ollama requires /v1 suffix (e.g. http://localhost:11434/v1){Colors.RESET}"
            )
        print()
        url_input: str = input(
            f"{Colors.WHITE}Enter Base URL (press Enter to keep current): {Colors.RESET}"
        ).strip()
        if url_input:
            base_url = url_input

        set_config_value("OPENAI.baseURL", base_url)
        set_config_value("OPENAI.key", "ollama")
        print(f"{Colors.GREEN}✓ Base URL saved: {base_url}{Colors.RESET}")
        print()

        print(f"{Colors.CYAN}Fetching available models from Ollama...{Colors.RESET}")
        print()

        from ..core.ai import list_ollama_models

        models: List[dict] = list_ollama_models()

        if not models:
            print(f"{Colors.YELLOW}Could not fetch models from Ollama.{Colors.RESET}")
            print(
                f"{Colors.WHITE}Make sure Ollama is running and you have pulled some models.{Colors.RESET}"
            )
            print()
            model_input: str = input(
                f"{Colors.WHITE}Enter model name manually: {Colors.RESET}"
            ).strip()
            if model_input:
                set_config_value("OPENAI.model", model_input)
                print(f"{Colors.GREEN}✓ Model saved: {model_input}{Colors.RESET}")
            return

        print(f"{Colors.WHITE}Available models:{Colors.RESET}")
        print()

        name_col: int = min(max((len(m["id"]) for m in models), default=30), 45)
        print(
            f"  {'#':>2}  {Colors.WHITE}{'Model':<{name_col}}{Colors.RESET}"
            f"  {Colors.CYAN}{'SIZE':>10}{Colors.RESET}"
            f"  {'MODIFIED':<12}"
        )
        print(f"  {'─' * (name_col + 28)}")
        ...
        for i, m in enumerate(models, 1):
            mid: str = m["id"]
            display: str = mid if len(mid) <= name_col else mid[: name_col - 2] + ".."
            size_str: str = _format_size(m.get("size", 0))
            modified_str: str = _format_ollama_date(m.get("modified_at", ""))
            print(
                f"  {i:>2}  {Colors.WHITE}{display:<{name_col}}{Colors.RESET}"
                f"  {Colors.CYAN}{size_str:>10}{Colors.RESET}"
                f"  {modified_str:<12}"
            )

        print()
        print(f"  0. Type manually / Cancel")
        print()

        selected: Optional[str] = None
        while selected is None:
            sel: str = input(
                f"{Colors.WHITE}Select model (0 to cancel): {Colors.RESET}"
            ).strip()

            if sel == "0":
                print(f"{Colors.YELLOW}Configuration cancelled.{Colors.RESET}")
                return
            elif sel.isdigit():
                idx: int = int(sel) - 1
                if 0 <= idx < len(models):
        ...
        print()
        print(f"{Colors.CYAN}Fetching details for {selected}...{Colors.RESET}")

        from ..core.ai import get_ollama_model_details

        details: Optional[dict] = get_ollama_model_details(selected)
        context_length: Optional[int] = None
        if details:
            context_length = details.get("context_length")
            ctx: str = f"{context_length:,}" if context_length else "-"
            params: str = details.get("parameters", "-")
            fmt: str = details.get("format", "-")
            print()
            print(f"{Colors.WHITE}{selected}{Colors.RESET}")
            print(f"  Context length: {Colors.CYAN}{ctx}{Colors.RESET}")
            print(f"  Parameters:     {Colors.CYAN}{params}{Colors.RESET}")
            print(f"  Format:         {Colors.CYAN}{fmt}{Colors.RESET}")
        else:
            print(
                f"{Colors.YELLOW}Could not fetch details for {selected}{Colors.RESET}"
            )

        print()
        if context_length is None:
            ctx_input: str = input(
                f"{Colors.WHITE}Enter context length (default: 4096): {Colors.RESET}"
            ).strip()
            if ctx_input.isdigit():
                context_length = int(ctx_input)
            elif not ctx_input:
                context_length = 4096

        confirm: str = (
            input(f"{Colors.WHITE}Confirm selection? (y/n): {Colors.RESET}")
            .strip()
            .lower()
        )

        if confirm != "y":
            print(f"{Colors.YELLOW}Selection cancelled.{Colors.RESET}")
            return

        set_config_value("OPENAI.model", selected)
        print()
        print(f"{Colors.GREEN}✓ Ollama configuration completed{Colors.RESET}")
        print(f"{Colors.WHITE}Base URL: {base_url}{Colors.RESET}")
        print(f"{Colors.WHITE}Model: {selected}{Colors.RESET}")
    ...
    def action_show_help() -> Literal[False]:
        ...
        print(
            f"  {Colors.YELLOW}OPENAI.key{Colors.RESET}      - API key (Groq / OpenRouter / OpenAI)"
        )
        ...
        print()
        print(f"{Colors.WHITE}Ollama (local) example:{Colors.RESET}")
        print(f'{Colors.CYAN}  igv config set OPENAI.key "ollama"{Colors.RESET}')
        print(
            f'{Colors.CYAN}  igv config set OPENAI.baseURL "http://localhost:11434/v1"{Colors.RESET}'
        )
        print(f'{Colors.CYAN}  igv config set OPENAI.model "llama3.2"{Colors.RESET}')
        print()
    ...
    config_menu.add_item(
        "2", "Configure AI (Groq/OpenRouter/OpenAI/Ollama)", action_configure_ai
    )
