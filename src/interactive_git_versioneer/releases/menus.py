...
def run_changelog_submenu(repo: git.Repo) -> bool:
    ...
    def action_auto_ai():
        ...
        action_generate_all_changelogs_with_ai(
            repo, rebuild=False, use_tag_messages=True
        )
    ...
    def action_rebuild_with_ai():
        ...
        if confirm == "s":
            ...
            action_generate_all_changelogs_with_ai(
                repo, rebuild=True, use_tag_messages=True
            )
...
def run_releases_menu(repo: git.Repo) -> bool:
    ...
    def show_releases_status():
        ...
        if os.path.exists(changelog_path):
            ...
            print(
                f"  {Colors.CYAN}→ Ir a 'Changelogs' para generar desde tags{Colors.RESET}"
            )

        # Verificar si hay changelogs pendientes basados en tags
        import re as regex_module

        total_tags = len(list(repo.tags))
        changelog_tags_count = 0
        if os.path.exists(changelog_path):
            with open(changelog_path, "r", encoding="utf-8") as f:
                content = f.read()
            changelog_tags_count = len(
                regex_module.findall(r"##\s*\[([^\]]+)\]", content)
            )

        if total_tags > changelog_tags_count:
            pending = total_tags - changelog_tags_count
            print(
                f"  {Colors.YELLOW}⚠ {pending} changelog(s) pendiente(s) de generar{Colors.RESET}"
            )
