...
def input_multiline(default_text: str = "") -> str:
    """Permite editar texto multilínea.

    Args:
        default_text: Texto inicial para editar

    Returns:
        str: Texto editedo por el usuario (vacío si Cancelar)
    """
    print(f"{Colors.WHITE}=== EDITAR TEXTO ==={Colors.RESET}")
    print(
        f"{Colors.YELLOW}Ingrese nuevo texto (línea vacía para confirmar, Ctrl+C para cancelar):{Colors.RESET}"
    )
    print(f"{Colors.CYAN}--- Texto actual ---{Colors.RESET}")
    print(default_text)
    print(f"{Colors.CYAN}--- Fin ---{Colors.RESET}")
    print()
    ...
    try:
        ...
        print()
        print(f"{Colors.YELLOW}Edición cancelada{Colors.RESET}")
        return ""
...
def wait_for_enter_or_skip(skip_hint: bool = True) -> str:
    ...
    Returns:
        str: "continue" si presiona Enter
             ...
             "cancel" si Ctrl+C
             "edit" si e (editar texto)
    """
    if skip_hint:
        print(
            f"{Colors.CYAN}[Enter: continuar | e: editar | Ctrl+A: permitir todos | Ctrl+X: saltar resto | q: salir]{Colors.RESET}"
        )
