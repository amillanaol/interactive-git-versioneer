...
def run_tag_settings_menu() -> None:
    """Menú para configurar ajustes de tags."""
    from ..config import get_ini_value, get_tag_detail_level
    from ..core.ui import clear_screen, print_header, Colors

    while True:
        clear_screen()
        print_header("CONFIGURACIÓN DE TAGS")

        current_detail = get_tag_detail_level()
        print(
            f"{Colors.WHITE}Nivel de detalle actual: {Colors.CYAN}{current_detail}{Colors.RESET}"
        )
        print()
        print(
            f"{Colors.WHITE}1. Conciso{Colors.RESET}   - Solo conventional commit (ej: feat: add dark mode)"
        )
        print(f"{Colors.WHITE}2. Detallado{Colors.RESET} - Commit + descripción breve")
        print(
            f"{Colors.WHITE}3. Completo{Colors.RESET}  - Commit + descripción + archivos afectados"
        )
        print()
        ...
        from ..config import load_ini_config, save_config

        parser = load_ini_config()

        if choice == "1":
            if not parser.has_section("TAGS"):
                parser.add_section("TAGS")
            parser.set("TAGS", "detailLevel", "concise")
            print(f"{Colors.GREEN}✓ Nivel configurado: conciso{Colors.RESET}")
        elif choice == "2":
            if not parser.has_section("TAGS"):
                parser.add_section("TAGS")
            parser.set("TAGS", "detailLevel", "detailed")
            print(f"{Colors.GREEN}✓ Nivel configurado: detallado{Colors.RESET}")
        elif choice == "3":
            if not parser.has_section("TAGS"):
                parser.add_section("TAGS")
            parser.set("TAGS", "detailLevel", "comprehensive")
            print(f"{Colors.GREEN}✓ Nivel configurado: completo{Colors.RESET}")
        else:
...
def run_commits_submenu(
    ...
    menu.add_item("3", "Generar tags", action_auto_generate)
...
def run_tags_menu(
    ...
    def action_modify_local_tags():
        clear_screen()
        print_header("GESTIONAR TAGS LOCALES")
        print()
        print(f"{Colors.WHITE}1. Ver tags locales{Colors.RESET}")
        print(f"{Colors.WHITE}2. Modificar un tag{Colors.RESET}")
        print(f"{Colors.WHITE}3. Eliminar un tag{Colors.RESET}")
        print(f"{Colors.WHITE}0. Volver{Colors.RESET}")
        print()
        ...
        if choice == "1":
            ...
            from .tagger import run_modify_tag_submenu

            run_modify_tag_submenu(repo)
        elif choice == "3":
            print(f"{Colors.YELLOW}Funcionalidad en desarrollo{Colors.RESET}")
        wait_for_enter()
        return False

    def action_modify_remote_tags():
        clear_screen()
        print_header("MODIFICAR TAGS REMOTAMENTE")
        print()
        print(f"{Colors.WHITE}1. Sincronizar tags desde remoto{Colors.RESET}")
        print(f"{Colors.WHITE}2. Eliminar tag del remoto{Colors.RESET}")
        print(f"{Colors.WHITE}0. Volver{Colors.RESET}")
        print()
    ...
    def action_tag_settings():
        run_tag_settings_menu()
        return False
    ...
    menu.add_item("1", "Ver último tag", action_change_last_tag)
    menu.add_item("2", "Ver tags remotos", action_view_remote_tags)
    menu.add_item("3", "Gestionar tags localmente", action_modify_local_tags)
    menu.add_item("4", "Gestionar tags remotamente", action_modify_remote_tags)
    menu.add_item("5", "Configuración de tags (IA)", action_tag_settings)
    menu.add_item("6", "Generar tags", action_auto_generate)
    menu.add_item("7", "Subir tags a GitHub", action_push_tags)
    menu.add_item("c", "Limpiar TODOS los tags", action_clean_tags)
