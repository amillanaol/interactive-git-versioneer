...
def run_tag_management_submenu(repo, untagged_commits, dry_run, push):
    ...
    def action_modify_local_tags():
        """Gestiona tags localmente: ver, modificar, eliminar."""
        clear_screen()
    ...
    def action_modify_remote_tags():
        """Gestiona tags remotamente: sincronizar, eliminar."""
        clear_screen()
        print_header("GESTIONAR TAGS REMOTOS")
        print()
    ...
    def action_tag_settings():
        """Abre el menú de configuración de tags IA."""
        from .menus import run_tag_settings_menu
    ...
    def action_clean_duplicates():
        """Limpia tags duplicados que apuntan al mismo commit."""
        from .actions import clean_duplicate_tags

        clean_duplicate_tags(repo, include_remote=True)
        wait_for_enter()
        return False

    def action_tag_maintenance():
        """Submenú de mantenimiento de tags."""
        while True:
            clear_screen()
            print_header("MANTENCIÓN DE TAGS")
            print()
            print(f"{Colors.WHITE}1. Limpiar tags duplicados{Colors.RESET}")
            print(f"{Colors.WHITE}2. Gestionar tags localmente{Colors.RESET}")
            print(f"{Colors.WHITE}3. Gestionar tags remotamente{Colors.RESET}")
            print(f"{Colors.WHITE}4. Limpiar TODOS los tags{Colors.RESET}")
            print(f"{Colors.WHITE}0. Volver{Colors.RESET}")
            print()
            ...
            if choice == "0" or choice == "-back-":
                ...
                action_clean_duplicates()
            elif choice == "2":
                action_modify_local_tags()
            elif choice == "3":
                action_modify_remote_tags()
            elif choice == "4":
                action_clean_tags()
            else:
    ...
    def action_view_tags_submenu():
        """Submenú para visualizar tags."""
        while True:
            clear_screen()
            print_header("VISUALIZAR TAGS DEL REPOSITORIO")
            print()
            print(f"{Colors.WHITE}1. Ver último tag{Colors.RESET}")
            print(f"{Colors.WHITE}2. Ver tags locales{Colors.RESET}")
            print(f"{Colors.WHITE}3. Ver tags remotos{Colors.RESET}")
            print(f"{Colors.WHITE}0. Volver{Colors.RESET}")
            print()
            ...
            if choice == "0" or choice == "-back-":
                ...
                action_view_last_tag()
            elif choice == "2":
                action_view_local_tags()
            elif choice == "3":
                action_view_remote_tags()
            else:
                print(f"{Colors.RED}Opción inválida{Colors.RESET}")
        return False
    ...
    submenu.add_item("1", "Visualizar tags del repositorio", action_view_tags_submenu)
    submenu.add_item("2", "Configuración de tags (IA)", action_tag_settings)
    submenu.add_item("3", "Generar tags", action_generate_tags_ai)
    submenu.add_item("4", "Subir tags a GitHub", action_push_tags)
    submenu.add_item("5", "Mantención de tags", action_tag_maintenance)
