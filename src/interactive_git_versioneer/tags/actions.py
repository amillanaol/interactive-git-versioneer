...
def apply_tags(
    ...
    if not skip_dirty_check:
        ...
        if is_dirty:
            ...
            print(
                f"{Colors.RED}║  ⚠️  ADVERTENCIA: HAY CAMBIOS SIN COMMITEAR                ║{Colors.RESET}"
            )
            print(
                f"{Colors.RED}╚══════════════════════════════════════════════════════════════╝{Colors.RESET}"
            )
            print()
            print(
                f"{Colors.YELLOW}Hay cambios en el working directory que no han sido comiteados.{Colors.RESET}"
            )
            print(
                f"{Colors.YELLOW}Se recomienda commitear los cambios antes de crear tags.{Colors.RESET}"
            )
            print()
    ...
    for commit in processed_commits:
        if getattr(commit, "custom_version", None):
            commit_to_version[commit.hash] = commit.custom_version
            parsed = parse_version(commit.custom_version)
            if parsed:
                major, minor, patch = parsed
        elif commit.version_type == "major":
            major += 1
            minor = 0
            patch = 0
            next_ver = f"v{major}.{minor}.{patch}"
            commit_to_version[commit.hash] = next_ver
